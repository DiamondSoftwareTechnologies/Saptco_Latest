...
function ValidateTripsMultiple(strData) {
    var Data = JSON.parse(strData, distinations)
    for (var i = 1; i <= distinations; i++) {

        if (!IsValidID(Data.Destinations[i - 1].DepartureStation)) {
            return "ErrEnterDepartrureStations";
        }

        if (!IsValidID(Data.Destinations[i - 1].ArrivalStation)) {
            return "ErrEnterArrivleStations";
        }

        if (Data.Destinations[i - 1].DepartureStation == Data.Destinations[i - 1].ArrivalStation) {
            return "ErrSameStationsSelected";
        }


        if (!IsValidDate(Data.Destinations[i - 1].DepartureDate)) {
            return "ErrEnterDepartureDate";
        }
...
function ValidatePassengers(passengerData) {
    var name = passengerData.Name.split(' ');
    for (var i = 0; i < name.length; i++) {
        if (!IsValidString(name[i])) {
