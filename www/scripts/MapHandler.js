...
function onDeviceReady() {
    ...
    //             console('doneLoction');
    //         } else {
    //             cordova.plugins.diagnostic.requestLocationAuthorization(function (responce) {
...
myApp.onPageInit('LimoTrackMap', function (page) {
    ...
    function loadScript() {
        try {
            ...
            script.src = "https://maps.googleapis.com/maps/api/js?key=AIzaSyDSHomJ8LmiGZ7MgMYuVbGtshpCJlzZsXY&libraries=places&sensor=false&callback=initialize";
...
myApp.onPageInit('MapLocationSelect', function (page) {
    ...
    function getPlaceName(lat, lng, fieldToUpdate) {
        ...
        $$.ajax(
            {
                // url: "https://maps.googleapis.com/maps/api/place/search/json?location=" + lat + "," + lng + "&rankby=distance&types=establishment&sensor=false&key=AIzaSyB_D49vCsjRmTzjIdyNRmDMW3bfC2oxGWI",
                url: "https://maps.googleapis.com/maps/api/place/search/json?location=" + lat + "," + lng + "&rankby=distance&types=establishment&sensor=false&key=AIzaSyD8SmhMoKrFR436HGOOP-oCGbvt0Q6oGmY",
                method: "Get",
                headers:
                    {
